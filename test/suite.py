...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Perform LaGrit Tests",
        prog="./suite.py",
    )
    parser.add_argument(
        "-f",
        "--full",
        help="Runs the entire test suite, cancels other options",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-l",
        "--level",
        help="Designate level of testing",
        action="store",
        type=int,
        nargs=1,
        default=0,
    ...
    parser.add_argument(
        "-s",
        "--single",
        help="Specify by name a single test to run",
        action="store",
        type=str,
        default="",
    )
    parser.add_argument(
        "-cl",
        "--clean",
        help="Clean directories of previous test results; cleans default output file unless testfile is specified",
        action="store_true",
    )
    parser.add_argument(
        "-t",
        "--test",
        help="Runs tests on directories; option for out file tag - stdout_[testfile].txt",
        action="store_true",
    )
    parser.add_argument(
        "testfile",
        help="Name <tag> for the test's out file stdout_<tag>.txt; default - <tag> = os name",
        nargs="?",
    ...
    parser.add_argument(
        "-c",
        "--check",
        help="Checks output files of tests; option for specific directory name [checkdir]",
        action="store_true",
    )
    parser.add_argument(
        "checkdir",
        help="Target dir for check function; default - recurse through current dir",
        action=lg_test.readable_dir,
        default=os.curdir,
        nargs="?",
    )
    parser.add_argument(
        "-exe",
        "--executable",
        help="Path to executable for testing",
        action="store",
        type=str,
        default=xlagrit,
    )
    parser.add_argument(
        "-fl",
        "--flags",
        help="Command line flags to pass to LaGriT on run",
        action="store",
        type=str,
        default="-log logx3dgen -out outx3dgen",
    )
    parser.add_argument(
        "-hf",
        "--hard_fail",
        help="Quits and returns non-zero exit code on failed test",
        action="store",
        type=int,
        nargs=1,
        default=1,
    ...
    if not (
        args.level or args.full or args.clean or args.test or args.check or args.single
    ):
    ...
    if args.full:
        ...
        for level in testable_levels:
            ...
            params = ["clean" * args.clean, "test" * args.test, "check" * args.check]
