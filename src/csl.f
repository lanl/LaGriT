*dk,csl
      subroutine csl(cmo,tollength,tolar,toldamage,mpary_in,mpno_in,
...
c     (iii) are attached to some element with aspect ratio less than
c           TOLAR ("shards").
c
c     The process is recursive in that new nodes are added to the
c     list of mass points, meaning that newly created shard edges can
c     be refined until all the shard edges in the mesh have length
c     less than TOLLENGTH.  Also, all shard edge refinement obeys
c     the 'Rivara Principle' in the sense that a refined shard edge should
c     be the longest shard edge in ANY element that contains it.  This
...
c       TOLAR - maximum element aspect ratio defining a "shard"
...
CPVCS
CPVCS       Rev 1.1   Thu Apr 06 08:17:36 2000   dcg
CPVCS     remove get_info_i calls
CPVCS
CPVCS       Rev 1.0   Fri Oct 30 15:27:06 1998   kuprat
...
c #####################################################################
     ...
     &   len_elements,node,node1,ierr
 
      real*8 ascend,tollength,rlong,rlen,toldamage,tolar,epsln,ar
      integer i1,i2,i3,i4
...
c
      ...
      if(ierrdum.ne.0) call x3d_error('csl', 'unpacktp')
     ...
     &      ,icscode)
      ...
      do i=1,nelements
         ...
         if (ityp.eq.ifelmtri) then
            call aratio_tri(xic(i1),yic(i1),zic(i1),xic(i2),yic(i2)
     &         ,zic(i2),xic(i3),yic(i3),zic(i3),ar)
         else
            i4=itet(4+itetoff(i))
            call aratio_tet(xic(i1),yic(i1),zic(i1),xic(i2),yic(i2)
...
c.... outer boundary.
 ...
 500           continue
     ...
     &                           .maxpar) then
                           ...
                           enddo
                           write(logmess,'(a)') 'CSL:Topological error'
                           call writloga('default',0,logmess,0,ierrw)
