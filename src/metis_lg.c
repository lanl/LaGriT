...
-----Original Message-----
From: Carl W. Gable [mailto:gable@lanl.gov] 
Sent: Thursday, November 06, 2008 8:09 AM
To: metis@cs.umn.edu
Subject: permission to distribute ?

In either case, permission to distribute/include METIS with your 
application must be obtained by sending email to metis@cs.umn.edu.
...
I would like to include METIS inside a mesh generation and setup code, 
LaGriT (http://lagrit.lanl.gov). At present I have written an interface 
to METIS (http://lagrit.lanl.gov/docs/metis.html) so that I can call 
METIS from within the LaGriT code. However, I do not distribute LaGriT 
with METIS. I have instead created dummy stubs that substitute for the 
METIS routines and I tell folks that if they want to use METIS, they 
must compile the code themselves. That is rather cumbersome. I get 1-3 
requests for LaGriT each week and in the last month a few users have 
been wanting to do bandwidth reduction via reordering. LaGriT is free.
